...
- **WebSocket broadcast topology** (2026-05-07) — single `tokio::sync::broadcast::channel` for default-window (5-min) clients. One server-side producer ticks every 2s, builds the snapshot once, sends; N clients subscribe and forward. When no one's connected, broadcast::send returns Err and we skip the work. Non-default windows (slider extended past 5m) fall back to per-connection timers.
- **BDS data on aircraft detail page** (2026-05-07) — tracker absorbs `comm_b: Option<CommBRegister>` payloads onto `AircraftState` per-register slots so a fresh BDS 5,0 doesn't clobber the last BDS 4,0. `/api/aircraft/<icao>` JSON adds a `comm_b: { bds40, bds50, bds60, last_seen }` block when the live tracker is attached. `/aircraft/<icao>` detail page renders each populated field as info-grid items (Selected Alt MCP, True Airspeed, Magnetic Heading, Mach, Roll Angle, etc.).
- **Statistical anomaly baseline scorer** (2026-05-07) — `adsb-server::baseline::BaselineCache` holds a spatial position-density grid (0.1° cells). Background task refreshes hourly from a `position_density_grid()` SQL aggregation over the last 7 days. `score(lat, lon)` returns `-log((count + 1) / (total + num_cells))` (Laplace-smoothed log-probability), clamped [0, 5]. Combined with the rules-based score on every position write — combined value lands in the `anomaly_score` column.
- **Anomaly score on dashboard** (2026-05-07) — `AircraftState.last_anomaly_score` carries the rules-based score from the most recent position update through the live-tracker view. `/api/positions` exposes `anomaly_score` per row. Map popups render an Anomaly line color-coded by tier (low ≥ 0.5, med ≥ 2.0, high ≥ 4.0); table rows get a per-tier background tint and a "!" marker.
- **`/api/stats` continuous aggregate** (2026-05-07) — `position_count_hourly` TimescaleDB matview precomputes hourly position counts. The 24-hour stat is `SUM(cnt)` over 24 rows instead of `COUNT(*)` over millions; real-time aggregates (`materialized_only = false`) cover the partial current hour via UNION with the base hypertable. Falls back to direct `COUNT(*)` when the matview isn't installed.
