...
- **Splatlas integration** — Captured 3D Gaussian splat scenes (from [Splatlas](https://splatlas.blueoctopustechnology.com/)) appear as pulse-ring markers with concentric copper dome circles showing the observation airspace radius. Click a marker → vantage buttons deep-link into the 3D viewer with a cinematic fly-in animation. **Receiver sites** — a scan that is also a physical ADS-B receiver (e.g. the Cybertruck) — render as a distinct cyan antenna pin with *no* coverage rings (it's the signal source, a point), linking into its splat.
